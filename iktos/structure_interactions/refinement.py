...
def refine_hydrophobics(
    ...
    for h, s in product(hydrophobics_all, pi_stackings_all):
        ...
        s_rec_ids = {i.unique_id for i in s.receptor}
        if h_rec_ids.isdisjoint(s_rec_ids):
            continue
        h_lig_ids = {i.unique_id for i in h.ligand}
        s_lig_ids = {i.unique_id for i in s.ligand}
        if h_lig_ids.isdisjoint(s_lig_ids):
            continue
        logger.debug(
            f'Removing hydrophobic: {(h.receptor[0].unique_id, h.ligand[0].unique_id)}'
        )
        filter_out.append(h)
    selection1 = [contact for contact in hydrophobics_all if contact not in filter_out]

    # Remove hydrophobic contacts for atoms that also interact via stacking or pi-hydrophobic
    filter_out = []
    for h, p in product(selection1, pi_hydrophobics_all):
        h_rec_ids = {i.unique_id for i in h.receptor}
        p_rec_ids = {i.unique_id for i in p.receptor}
        if h_rec_ids.isdisjoint(p_rec_ids):
            continue
        h_lig_ids = {i.unique_id for i in h.ligand}
        p_lig_ids = {i.unique_id for i in p.ligand}
    ...
    for h1, h2 in combinations(selection2, 2):
        if h1 in filter_out or h2 in filter_out:
            continue
        h1_lig_ids = {i.unique_id for i in h1.ligand}
        h2_lig_ids = {i.unique_id for i in h2.ligand}
        h1_rec_ids = {i.unique_id for i in h1.receptor}
        h2_rec_ids = {i.unique_id for i in h2.receptor}
...
def refine_pi_cations(
    ...
    for c, s in product(pi_cations_all, pi_stackings_all):
        ...
        if c_lig_ids.isdisjoint(s_lig_ids):
            continue
        logger.debug(
            f'Removing pi-cation: {(c.receptor[0].unique_id, c.ligand[0].unique_id)}'
        )
        filter_out.append(c)
...
def refine_pi_hydrophobics(
    ...
    for p1, p2 in combinations(pi_hydrohobics_all, 2):
        if p1 in filter_out or p2 in filter_out:
            continue
        p1_lig_ids = {i.unique_id for i in p1.ligand}
        p2_lig_ids = {i.unique_id for i in p2.ligand}
        p1_rec_ids = {i.unique_id for i in p1.receptor}
        p2_rec_ids = {i.unique_id for i in p2.receptor}
        if p1_lig_ids == p2_lig_ids:
            if p1.receptor[0].residue_id == p2.receptor[0].residue_id:
                p = p1 if p1.offset > p2.offset else p2
            else:
                continue
        elif p1_rec_ids == p2_rec_ids:
            if p1.ligand[0].residue_id == p2.ligand[0].residue_id:
                p = p1 if p1.offset > p2.offset else p2
            else:
                continue
        else:
            continue
        logger.debug(
            f'Removing pi-hydrophobic: {(p.receptor[0].unique_id, p.ligand[0].unique_id)}'
        )
        filter_out.append(p)
...
def refine_h_bonds(
    ...
    for h, b in product(h_bonds_all, salt_bridges_all):
        h_rec_ids = {i.unique_id for i in h.receptor}
        b_rec_ids = {i.unique_id for i in b.receptor}
        if h_rec_ids.isdisjoint(b_rec_ids):
            continue
        h_lig_ids = {i.unique_id for i in h.ligand}
        b_lig_ids = {i.unique_id for i in b.ligand}
        if h_lig_ids.isdisjoint(b_lig_ids):
            continue
        logger.debug(
            f'Removing H-bond: {(h.receptor[0].unique_id, h.ligand[0].unique_id)}'
        )
        filter_out.append(h)
